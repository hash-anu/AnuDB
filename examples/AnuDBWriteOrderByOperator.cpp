...
int main() {
    ...
    // STEP 2: Create indexes for fields we'll sort by
    std::cout << "\n===== Creating Indexes =====\n";

    // Create index for price field
    status = products->createIndex("price");
    if (!status.ok()) {
        if (status.message().find("already exists") != std::string::npos) {
            std::cout << "Index on 'price' already exists." << std::endl;
        }
        else {
            std::cerr << "Failed to create index on price: " << status.message() << std::endl;
            return 1;
        }
    }
    else {
        std::cout << "Index on 'price' created successfully." << std::endl;
    }

    // Create index for rating field
    status = products->createIndex("rating");
    if (!status.ok()) {
        if (status.message().find("already exists") != std::string::npos) {
            std::cout << "Index on 'rating' already exists." << std::endl;
        }
        else {
            std::cerr << "Failed to create index on rating: " << status.message() << std::endl;
            return 1;
        }
    }
    else {
        std::cout << "Index on 'rating' created successfully." << std::endl;
    }

    // STEP 3: Execute $orderBy queries
    std::cout << "\n===== $orderBy Operator Examples =====\n";
    ...
    std::cout << "\n----- Query: Order By Price (Ascending) -----\n";

    docIds = products->findDocument(orderByPriceAsc);
    ...
    // Example 2: Sort products by rating descending
    ...
    docIds.clear();
    std::cout << "\n----- Query: Order By Rating (Descending) -----\n";

    docIds = products->findDocument(orderByRatingDesc);
