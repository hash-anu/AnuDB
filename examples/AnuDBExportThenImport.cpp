...
int main() {
    ...
    status = db.exportAllToJsonAsync("users", "./user_example/");
    if (!status.ok()) {
        std::cerr << "Failed to export documents: " << status.message() << std::endl;
    }
    else {
        std::cout << "Documents exported successfully to ./user_example/" << std::endl;
    }
    // since export operation is thread we need wait for it finish
    users->waitForExportOperation();

    status = db.createCollection("users1");
    anudb::Collection* users1 = db.getCollection("users1");
    status = db.importFromJsonFile("users1", "./user_example/users.json");
    if (!status.ok()) {
        std::cerr << "Failed to import documents: " << status.message() << std::endl;
    }
    else {
        std::cout << "Documents imported successfully to users1 collection" << std::endl;
    }

    users1 = db.getCollection("users1");

    // Create a new document object to store retrieved data
    anudb::Document doc1;

    // Read the document with ID "user001" from the collection
    status = users1->readDocument("user001", doc1);
